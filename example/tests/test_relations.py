...
class TestHyperlinkedFieldBase(TestBase):
    def setUp(self):
        ...
        self.request = RequestFactory().get(
            reverse("entry-detail", kwargs={"pk": self.entry.pk})
        )
        self.view = EntryViewSet(
            request=self.request, kwargs={"entry_pk": self.entry.id}
        )
...
class TestHyperlinkedRelatedField(TestHyperlinkedFieldBase):
    def test_single_hyperlinked_related_field(self):
        ...
        links_expected = {
            "self": "http://testserver/entries/{}/relationships/blog".format(
                self.entry.pk
            ),
    ...
    def test_many_hyperlinked_related_field(self):
        ...
        self.assertRaises(
            NotImplementedError, field.to_representation, self.entry.comments.all()
        )
        self.assertRaises(SkipField, field.get_attribute, self.entry)

        links_expected = {
            "self": "http://testserver/entries/{}/relationships/comments".format(
                self.entry.pk
            ),
...
class TestSerializerMethodHyperlinkedRelatedField(TestHyperlinkedFieldBase):
    def test_single_serializer_method_hyperlinked_related_field(self):
        serializer = EntryModelSerializerWithHyperLinks(
            instance=self.entry, context={"request": self.request, "view": self.view}
        )
